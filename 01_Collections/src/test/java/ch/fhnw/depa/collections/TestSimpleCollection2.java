...
public class TestSimpleCollection2 {

	private SimpleCollection2<Integer> col;

	@Before
	public void setUp() throws Exception {
		col = new SimpleCollection2<>();
	}
	...
	public void testIterator() {
		...
		it.next(); // provoke a no such element exception
	}
	...
	public void testIteratorRemove() {
		col.add(3);
		col.add(5);
		testExceptionsCompliantWithRemove();
		Iterator<Integer> it = col.iterator();
		it.next();
		it.remove();
		assertEquals(0, col.size());
	...
	public void testRemove() {
		assertEquals(0, col.size());
		assertFalse(col.remove(42));
		assertEquals(0, col.size());

		col.add(2);
		assertTrue(col.remove(2));
		assertEquals(0, col.size());
		
		col.add(3);
		col.add(5);
		assertTrue(col.remove(3));
	...
	private void testExceptionsCompliantWithRemove() {
		try {
			Iterator<Integer> it = col.iterator();
			it.remove();
			fail("IllegalState- or UnsupportedOperationException expected");
			...
			try {
				...
				fail("Do not throw IllegalStateExceptions if removal is not supported!");
			}
		} catch (UnsupportedOperationException uoe) {
