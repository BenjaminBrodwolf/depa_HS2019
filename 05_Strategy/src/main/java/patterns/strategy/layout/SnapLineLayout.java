...
public class SnapLineLayout implements LayoutManager {
	...
	public void layoutContainer(Container parent) {
		...
		width = parent.getSize().width - parent.getInsets().left - parent.getInsets().right;
		...
		for (int i = 0; i < numberOfComponents; i++) {
			...
			x = xpos + left.abs + (int) (left.rel * width);
			w = xpos + right.abs + (int) (right.rel * width);
			y = ypos + top.abs + (int) (top.rel * height);
			h = ypos + bottom.abs + (int) (bottom.rel * height);
			w -= x;
			h -= y;
	...
	public void registerComponent(Component component, String top, String bottom, String left, String right) {
		...
		components.put(component, new SnapLineBoundingBox(top, bottom, left, right));
	}
